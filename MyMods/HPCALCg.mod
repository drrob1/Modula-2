...
(*
  ...
  15 Oct 13 -- converted to gm2.
*)
  IMPORT LowLong,SYSTEM,ASCII;
  FROM SYSTEM IMPORT CAST;
  FROM Environg IMPORT GetCommandLine;
  FROM UTILLIBg IMPORT NULL,STRTYP,BUFSIZ,BUFTYP,STR10TYP,TRIM,STRCMPFNT;
  FROM REALLIBg IMPORT AINT,AMOD,ROUND,PWRI,GETCROPNUM;
  FROM SysClock IMPORT DateTime,GetClock,CanGetClock,CanSetClock,IsValidDateTime,SetClock;
  FROM LongMath IMPORT sqrt,exp,ln,sin,cos,tan,arctan,arcsin,arccos,power,round,pi;
  FROM LowLong IMPORT trunc; (* sign,ulp,intpart,fractpart,round *)
  ...
  FROM MiscStdInOutg IMPORT WriteCard, WriteLongCard, CLS, WriteString, WriteLn, PressAnyKey, Error, WriteInt,
      WriteReal, WriteLongReal, WriteChar, ReadChar, ReadString, ReadCard, ReadLongReal;
(*
...
*)
    ...
    | OP  : IF (I = 6) OR (I = 20) THEN SWAPXY;
         ELSE
           ...
           | 22 : (* %   *) STACK[X] := STACK[Y] * STACK[X] / 100.0;
           ELSE
...
' @, LASTX -- put the value of the LASTX register back into the X register.');
                  WriteLn;
                  WriteString(
' !,DN,ROLLDN -- roll the stack down one register.  X goes to T1.');
                  WriteLn;
                  WriteString(
' D -- Dump2console.   DUMP -- dump the stack to the terminal.');
                  WriteLn;
                  WriteString(
...
' , -- stack up.  ! -- stack down.');
                  WriteLn;
                  WriteString(
' HCF -- Push HCF(Y,X) onto stack without removing Y or X.');
                  WriteLn;
...
(*                  STACK[X] := LowLong.round(STACK[X],1); doesn't work *)
                  IF STACK[X] >= 0.0 THEN           (* X is positive or zero *)
                ...
                ELSIF STRCMPFNT(TOKEN.CHARS,'HEX') = 0 THEN  (* treat as positive *)
                  IF (STACK[X] > 0.0) AND (STACK[X] <= 1.80E19) THEN
                    LC := TRUNC(STACK[X]);
                    LongCard2HexStr(LC,str);
                    WriteString(' Value of x reg in hex: ');
                    WriteString(str);
                    WriteLn;
                  ELSIF (STACK[X] < 0.0) AND (STACK[X] >= -2.0E9) THEN
                    I := TRUNC(STACK[X]-0.005);
                    LC := VAL(LONGCARD,I);
                    LongCard2HexStr(LC,str);
                    WriteString(' Value of x reg in hex: ');
                    WriteString(str);
                    WriteLn;
                  ELSE
                    WriteString(' Cannot convert to hex string, as number is out of range.');
                    WriteLn;
                  END;
