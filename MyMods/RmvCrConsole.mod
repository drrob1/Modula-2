...
(*
  ...
  21 Aug 11 -- Made output ext .linx to reflect output file format.
*)
...
*)
  ...
  IF c = 0 THEN
    ...
    IF LENGTH(OutBuf) = 0 THEN
      ConstructFileName(InBuf,'.linx',outname);
    ELSE
        ConstructFileName(OutBuf,'.linx',outname);
    END;

  ELSE
        ConstructFileName(inputline,'*.txt',inname);
        ConstructFileName(inputline,'*.linx',outname);
