...
(*
  ...
  31 Jul 04 -- Needed to change logic because Citibank now d/l null fields that I need.
   8 Aug 04 -- Fixed a minor output file bug.
  ...
  17 Jun 06 -- Changed initial value of chknum to zero, and any key will exit now.  Stopped
                the file output of "Authorized Transfer" phrase.
  27 Jan 07 -- Decided to stop maintaining this version.  See CitiFilterTW.
*)
  IMPORT MiscM2;
...
(* TOKEN is a date string and can be processed as such *)
    ...
    GETTKN(tpv,TOKEN,TKNSTATE,I,RETCOD);  (* Was a null field that needed to be transnum, *)
                                          (* back TO transnum again.                      *)
(*    ExtractLastNum(TOKEN,transnum);                                                             *)      
(*    FindNext("CASH WITHDRAWAL AT",TOKEN.CHARS,0,ok,discardthis);  Not needed in Nov 04 revision *)
(*    IF (transnum = 0) OR ok THEN                                                                *)
   ...
   (*    TRIM(TOKEN);  Not needed after Citibank divided this field as they removed most extra spaces *)
    ...
    WriteString(tw,TOKEN.CHARS,a);  (* Will only display 2nd half of description, but it's enough *)
    ...
    GETTKN(tpv,TOKEN,TKNSTATE,I,RETCOD);  (* discard null field item *)
