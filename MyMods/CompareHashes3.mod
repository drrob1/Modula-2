...
MODULE CompareHashes3;
(*
  REVISION HISTORY
  ----------------
   6 Apr 13 -- 2nd version of module, using CompareHashes as a template.  I will use VLI to compare all digits
                of the hashes.
  20 Apr 13 -- Now up to 3rd version of module.  Console mode file picker now to be used.
...
(* Have HashesListFile name in InName.  Must parse it into the 2 parts *)
  ASSIGN2BUF(InNameStr,INFNAM);
  FOPEN(HashesListFile,INFNAM,RD);
  ...
  REPEAT (* to read multiple lines *)
    stop := FALSE;
    filesize := 0;
    FRDTXLN(HashesListFile,INBUF,0);
...
*)
  ...
  UNTIL HashesListFile.FILE.eof;
(* outer LOOP to read multiple lines*);
  fclose(TargetFile);
  FCLOSE(HashesListFile);
  WriteLn;
