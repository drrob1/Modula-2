...
16 Sep 09 -- The new LCD monitors provide much more flare light to my mammo area than the old CRTs did.
              I decided to just have a little pgm to blank the monitors out, so that the orig FHPacs
              can still click the mouse and I won't get logged out if I read mammos for a while.
              And I slightly changed how timeoutreset is handled.
              And I learned the hard way that .res file needs ICONS, not bitmaps.  I cannot copy a
              bitmap to an icon.
--------------------------------------*)

MODULE FHScreenPower;
IMPORT SYSTEM;
...
*)
  InputPromptLn3 = ' <sp> sleep toggle; <F1> Emerg screen timer reset. ';
  LastMod = '18 Sep 09';
...
*)
    ...
    | TWM_CREATE:
        ...
        TimeOutReset := PowerTimeOut; (* This gave an assignment error on flushing computer.  I'm now more careful about -1 here *)
        IF TimeOutReset > 0 THEN DEC(TimeOutReset) END;
        WiggleMouse := TimeOutReset;
        sleep := FALSE;
...
*)
      IF msg.timerId = MouseTimer THEN
        ...
        ELSIF WiggleMouse <= 0 THEN (* WiggleMouse not allowed to change, so don't worry about it *)
          WiggleMouse := TimeOutReset;
...
*)
    Win := CreateWindow(NIL, (* parent : WinShell.Window *)
                        "FH Screen Power", (* name : ARRAY OF CHAR *)
                        Menu100,        (* menu : ARRAY OF CHAR *)
