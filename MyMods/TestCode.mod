...
(* CONST Caps= Strings.Capitalize; Length= Strings.Length; Delete= Strings.Delete; *)
  ...
  VAR
    b1,b2,b3,b4,b5,b6,PROMPT,NAMDFT,TYPDFT,INFNAM,OUTFNAM : BUFTYP;
...
(*
  ...
  s6[b2.COUNT+1] := NULL;
  WriteString("copyleft s5, copyright s6: ");
  WriteString(s5);
  WriteString(",  ");
  WriteString(s6);
  WriteLn;
  MRGBUFS(b1,b2);
  WriteString(" merge bufs b1 = b1 + b2: ");
  WriteString(b1.CHARS);
  WriteLn;
  LOOP
      WriteString(" enter ch to scanfwd in s1: ");
      ReadChar(ch);
      SkipLine;
      c1 := SCANFWD(ADR(s1),LENGTH(s1),ch,TRUE);
      c2 := SCANFWD(ADR(s1),LENGTH(s1),ch,FALSE);
      c3 := SCANBACK(ADR(s1),LENGTH(s1),ch,TRUE);
      c4 := SCANBACK(ADR(s1),LENGTH(s1),ch,FALSE);
      WriteString(" fwd true, fwd false, back true, back false :");
      WriteCard(c1,0);
      WriteCard(c2,0);
      WriteCard(c3,0);
      WriteCard(c4,0);
...
*)
  ...
  WriteLn;
  IF STRCMPFNT(INFNAM.CHARS,"QUIT.TXT")=0 THEN EXIT END;
