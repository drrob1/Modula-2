...
(*
  ...
  19 Aug 11 -- Using procedures from the debugged RmvCrConsole.
  21 Aug 11 -- Made the output ext .linx to reflect the output file format.
   7 Sep 11 -- Will use new FilePicker module instead of BasicDialogs open dialog.
  18 Dec 11 -- Made this behave like AddCr2 w/ new FileNamePicker and renaming of files
*)
...
(*
  PROCEDURE PromptOpenFile(VAR INOUT name : ARRAY OF CHAR;
   ...
   title = the caption text of the dialog. title can be empty "".  in this case the default operating system title is used.  If createable = TRUE then the file need not already exist, otherwise the file must exist for the dialog to return successful.
