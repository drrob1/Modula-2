...
*)
IMPORT  SeqFile, STextIO, TextIO, SWholeIO (*, SIOResult *);
TYPE 
...
BEGIN
  out.flags := SeqFile.text + SeqFile.old;  (* When I did not add the 'old' flag, the OpenWrite failed w/ a fileExists error *)
  ...
  IF in.openRes = SeqFile.opened THEN
    ...
    c := 0;
    LOOP
      TextIO.ReadRestLine(in.handle,in.str);
      in.strlen := LENGTH(in.str);
      TextIO.SkipLine(in.handle);
      IF in.strlen = 0 THEN EXIT END;
      INC(c);
      STextIO.WriteString(in.str);
      STextIO.WriteString(', len =');
      SWholeIO.WriteCard(in.strlen,2);
      STextIO.WriteLn;
    END; 
    SeqFile.Close(in.handle);
