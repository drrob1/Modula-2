...
15 Apr 07 -- Still haven't a handle on this occasional arith bug.  But added arrow key stack fcnts.
--------------------------------------*)
...
VAR
  ...
  yCaret  : INTEGER;
  ch1,ch2,ch3 :  CHAR;
  bool,inputprocessed,hpFileExists,RegAltered,StackFileExists : BOOLEAN;
  sigfig,c1,c2,c3    :  CARDINAL;
  inputline,HPFileName,StackFileName,Xstr,str1,str2,str3,str4,str5,str6,str7,str8,str9,str0 : STRTYP;
...
'    Must be first cmd on line for this, else reg from a different module is used.';
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr :=
' SWAP,SWAPXY,<>,><,~,`, side arrow keys -- equivalent commands that swap the X and Y registers.';
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr :=
...
' ROLLDN -- roll the stack down one register.  X goes to T1.';
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr := 
' Up or Down arrow, PageUp, Home, etc -- Stack movement commands done immediately.';
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr :=
...
' MOD -- evaluate Y MOD X, put result in X and pop stack 1 reg.';
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr :=
' SIN,COS,TAN,ARCTAN,ARCSIN,ARCCOS -- Now in degrees.';
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr :=
...
(* Now must process the cmd, which is the whole reason for this pgm existing! *)
        ...
        ELSE (* CASE ch *)
          ...
          ELSIF msg.k_ch = ';' THEN
            ch3 := '*';
(* Now comma can push stack, like extra press of ENTER key on the actual calculators
          ELSIF msg.k_ch = ',' THEN
            ch3 := '-';
*)
          ELSIF msg.k_ch = "'" THEN
