...
BEGIN
    ...
    TWM_CLOSE:
        ...
        DISPOSE(boolp);
        RETURN OkayToClose;
I need 2 counters, one for screen saver, SSTimeOut, which is ~5 min, and another for when
to exit so that the activate loop in the batch file can keep PowerScribe and Epic awake
    | TWM_CREATE:
        xCaret := 10;
        yCaret := 0;
        inputline := '';
