...
BEGIN
    ...
    IF PrivateFunction(crypt,
                       ...
                       crypt^.buffer, emLen)
    THEN
        RETURN Decode_OAEP(crypt^.buffer, emLen,
                           encodingParams, encodingParamsLen,
                           message, messageLen,
                           HashFunc_SHA1, crypt^.sha1, SHA1.HashLength);
    END;
...
BEGIN
    ...
    IF PublicFunction(crypt,
                      ...
                      crypt^.buffer, emLen)
    THEN
        RETURN Verify_PSS(crypt^.buffer, emLen,
                          saltLen,
                          messageHash,
                          HashFunc_SHA1, crypt^.sha1, SHA1.HashLength);
    END;
...
BEGIN
    ...
    IF valid THEN
        ...
        IF crypt <> NIL THEN
            ...
            IF PublicFunction(crypt, ADR(em), SIZE(em), ADR(output), outputLen) THEN
                    ...
                    IF verify(ct, output, SIZE(ct)) THEN
                        ...
                        THEN
                            IF outputLen = SIZE(em) THEN
                                valid := verify(output, em, SIZE(em));
                            END;
                        END;
                    END;
                END;
            ...
            IF valid THEN
                ...
                outputLen := SIZE(output);
                IF Encrypt_OAEP_SHA1(crypt,
                                     ADR(message), SIZE(message),
                                     NIL, 0,
                                     ADR(output), outputLen)
                THEN
                    msgLen := SIZE(msg);
                    IF Decrypt_OAEP_SHA1(crypt,
                                         ADR(output), outputLen,
                                         NIL, 0,
                                         ADR(msg), msgLen)
                    THEN
