(*
  ...
  19-Dec-11  Changing display and select process so that the list remains on screen and default selection starts with
   ...
   9-Jun-12  Same code should now compile for Stony Brook and xds, at least as originally delivered before I added EOL code.
*)
...
BEGIN
    ...
    LOOP
        ...
        ReadChar(cmd);
        SkipLine;
(*        WriteString(' cmd is '); WriteString(cmd); WriteString('    '); WholeStr.IntToStr(ORD(cmd),s); WriteString(s); WriteLn; *)
        IF (cmd=EOL) OR (cmd=40C) THEN cmd := '1'; END;
        WriteLn;

(*        WriteString(' cmd is '); WriteString(cmd); WriteLn; *)
        CASE CAP(cmd) OF
        | "0" .. "9" :
