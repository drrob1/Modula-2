...
FROM SYSTEM IMPORT
    ADR, BYTE, ADDRESS, CAST, IsThread,
    EXCEPTADR, FUNC, OutputCallTrace;
...
FROM WinShell IMPORT
    Window, FontInfo, FontWeights, CharacterSets, WasSystemDialogBox,
...
FROM WINGDI IMPORT
    ...
    FW_LIGHT, FW_NORMAL, FW_DEMIBOLD, FW_BOLD, FW_HEAVY, FIXED_PITCH,
...
VAR
    ...
    %IF UNICODE %THEN
    exMess      : ARRAY [0..63] OF UCHAR;
    exMessA     : ARRAY [0..63] OF ACHAR;
    %ELSE
    exMess      : ARRAY [0..63] OF ACHAR;
    %END
...
PROCEDURE ConvertNumber(VAR numStr : ARRAY OF CHAR;
                        VAR val : CARDINAL) : BOOLEAN;
VAR
    l           : CARDINAL;
    ok          : BOOLEAN;
BEGIN
...
BEGIN
    ...
    |
    SYMBOL_CHARSET:
        lf.lfCharSet := WINGDI.SYMBOL_CHARSET;
    |
    DEFAULT_CHARSET:
    ...
    IF ChooseFont(cf) THEN
        ...
        font.fixedPitch := (FIXED_PITCH BAND lf.lfPitchAndFamily) <> 0;
        ...
        ANSI_CHARSET:
            font.charSet := LATIN1_CHARSET;
        |
        EASTEUROPE_CHARSET:
            font.charSet := LATIN2_CHARSET;
        |
        OEM_CHARSET:
            font.charSet := ASCII_CHARSET;
        |
