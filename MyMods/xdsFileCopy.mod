...
BEGIN
  ...
  Strings.Assign('copy ',cmd);
  Strings.Append(src.name,cmd);
  Strings.Append(' ',cmd);
  Strings.Append(dest.name,cmd);
  success := ProgExec.Command(cmd,c);
  IF success THEN
    STextIO.WriteString(' progexec succeeded with exit code of ');
    SWholeIO.WriteCard(c,2);
    STextIO.WriteLn;
  ELSE
    STextIO.WriteString(' progexec failed.  No clue as to why.');
    STextIO.WriteLn;
  END;
END CopyFile;
