...
23 Dec 20 -- Now called JHPacs.mod.  I'm going to allow it to read a duration from the command line.
--------------------------------------*)
<*/Resource:FHPacs.RES*>
MODULE JHPacs;
IMPORT SYSTEM;
...
VAR
  ...
  WiggleMouse     : INTEGER;
  tknptr          : TOKENPTR.TKNPTRTYP;
  token           : BUFTYP;
  tknState        : TOKENPTR.FSATYP;
...
BEGIN
    ...
    | TWM_CREATE:
        ...
        (*
        FUNC WINUSER.SystemParametersInfo(WINUSER.SPI_GETSCREENSAVETIMEOUT,c,ADR(SSTimeOut),c);  Now set in module's main() function.
...
(* Get and process the optional timer reset value on the command line. *)
  GetCommandLine(str9);                 (* From Environment import ... *)
  tknptr := TOKENPTR.NewToken(str9);

  LOOP
    TOKENPTR.GETTKN(tknptr, token, tknState, L, c3);
    IF c3 > 0 THEN
      TimeOutReset := EmergencyScreenReset;
      EXIT;
    END;

    IF tknState = TOKENPTR.DGT THEN
      IF (L <= 0) OR (L > 2000) THEN
        L := EmergencyScreenReset;
      END;
