...
BEGIN
    ...
    TWM_CLOSE:
        ...
        RETURN OkayToClose;
    | TWM_CREATE:
        FUNC SetWindowIcon(tw, SS7Icon16);
        ch1 := 'a';
        ch2 := 0c;
        ch3 := 0c;
        str1 := '';
...
(*        SSTimeOut := 5;  For testing purposes only *)
        SSTimeOut := 32*SSTimeOut; (* GE PACS uses 900 sec timeout=15 min.  Make it 8 hrs *)
...
*)
      IF msg.timerId = MouseTimer THEN
        ...
        ELSIF WiggleMouse <= 0 THEN  (* I don't think I want to WiggleMouse regardless of SS status *)
          WiggleMouse := SSTimeOut;
          WINUSER.mouse_event(WINUSER.MOUSEEVENTF_MOVE, CAST(DWORD, mousemoveamt),
                              CAST(DWORD, mousemoveamt), 0, 0);
          WINUSER.mouse_event(WINUSER.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0);
...
*)
    Win := CreateWindow(NIL, (* parent : WinShell.Window *)
                        "SS7 Dancing Mouse TW", (* name : ARRAY OF CHAR *)
                        "",        (* menu : ARRAY OF CHAR *)
                        SS7Icon16, (* icon : ARRAY OF CHAR *)
