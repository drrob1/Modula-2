...
BEGIN
    j := SIZE(num) * 8;
    FOR i := 0 TO SIZE(num)-1 DO
        j := j - 8;
        db := (num SHR j) BAND 0FFh;
        data[i] := db;
    END;
END Card32ToBytes;
...
BEGIN
    IF (hashLen <= MaxHashLen) AND (emLen >= messageLen+(2*hashLen)+1) THEN
        ...
        IF PS <> 0 THEN
            FillMemBYTE(ADR(emP^[hashLen]), PS, 0);
        END;
        emP^[hashLen+PS] := 1;
...
BEGIN
    IF (hashLen <= MaxHashLen) AND (emLen > ((2*hashLen)+1)) THEN
        ...
        MoveMem(ADR(seedMask), maskedDB, hashLen);
        IF DataBlocksEqual(ADR(seedMask), ADR(seed), hashLen) THEN
