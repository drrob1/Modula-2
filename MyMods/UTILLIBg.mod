IMPLEMENTATION MODULE UTILLIBg;
...
Copyright (C) 1987  Robert Solomon MD.  All rights reserved.
  ...
  24 Aug 04 -- Added SubStrCMPFNT.
   ...
   4 Oct 13 -- Converted to gm2.  And changed name of module.
  13 Oct 13 -- Added MINCARDFNT.
*)

  IMPORT ASCII;
  FROM SYSTEM IMPORT ADDRESS;
  FROM MiscStdInOutg IMPORT WriteString, WriteLn, WriteCard;

  FROM TOKENIZEg IMPORT FSATYP,DELIMCH,INI1TKN,GETTKN,GETTKNSTR;
  FROM Environg IMPORT GetCommandLine;

(*
    The following constants and types are declared in the DEFINITION MODULE.
CONST
    BUFSIZ    = 255;
    CR        = 15C; /* CHR(13) */
    LF        = 12C; /* CHR(10) */
...
/* The type has one more than BUFSIZ so array subscript errors are avoided. */
  STRTYP = ARRAY [1..BUFSIZ+1] OF CHAR;
  BUFTYP = RECORD
    CHARS : STRTYP;
...
*)
TYPE
  ChrArrayType   = ARRAY [0..65535] OF CHAR;
  pChrArray   = POINTER TO ChrArrayType;
...
BEGIN
(*
  Because the strlenfnt returns the actual count of characters, and here we are using it to search a zero origin character array, the STRLENFNT will
  return a value that is 1 too high for our purposes.  The algorithm must compensate for this in the conditional tests of the main while loop below.
*)
  ...
  END(*WHILE*);
(*
  Strings are only equal if their lengths are also equal.  Otherwise one is a substring of the other.  
  In this case, the shorter string is less than the longer one.
*)
...
TRIM FUNCTION.
  ...
  first non-blank character.  That is, trailing blanks are deleted, or trimmed.  A length of zero means a blank line was input.
...
BEGIN
  ...
  WHILE (COL > 0 ) AND (BUF.CHARS[COL] = ASCII.sp) DO DEC(COL); END(*WHILE*);
  RETURN(COL);
END TRIMFNT;
...
TRIM Procedure.
  ...
  Also, the NULL character is put in the proper position in the CHARS field to delete the trailing blanks altogether.
...
(*
  The separate test against BUF.COUNT + 1 is necessary to allow the last chars in the buffer to be removed.  In this case, no chars need be
...
(* Need separate test, else Nbytes-1 would be -1 which is not allowed as Nbytes is a cardinal *)
...
BEGIN
     IF Nbytes=0 THEN RETURN; END(*IF*);
(* Need separate test, else Nbytes-1 would be -1 which would error in cardinal arithmetic *)
...
********************************** SCANBACK *******************************
Scan Backward.
Scans backward Nbytes number of bytes for byte value of ch, returns number of bytes scanned if successful, and 0 if unsuccessful.  
Stops at equality if equal=TRUE, and at inequality if equal=FALSE.
Bypasses type, range & index checking -- Use with care.
Assume Destination to be of SIZE at least Nbytes.  That rules out the possibility of running up against segment boundaries.
*)
