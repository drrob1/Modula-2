...
(*        SetScrollDisableWhenNone(tw,TRUE,TRUE); *)
        ...
        juldate3 := 0;
        chknum := 0;
        FUNC FileFunc.DeleteFile('ESAV466.ASC');
        FUNC FileFunc.DeleteFile('ESAV.ASC');
        ok := FileFunc.CopyFile('SAV_466.CSV','ESAV466.ASC');
        IF NOT ok THEN
          MiscM2.Error(' Could not copy SAV_466.CSV file.  Does it exist?');
          HALT;
        END(*if*);
        ASSIGN2BUF('ESAV466.ASC',INFNAM);
        ASSIGN2BUF('ESAV.ASC',OUTFNAM);
    ...
    TWM_PAINT:
        ...
        FCLOSE(OUTUN1);
        WriteLn(tw);
        WriteString(tw,' eSAV file now closed.',a);
        WriteLn(tw);
        EraseToEOL(tw,a);
        WriteLn(tw);
        WriteLn(tw);
