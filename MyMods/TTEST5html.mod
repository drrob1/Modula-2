MODULE TTEST5html;
(*
Created 2/9/2009 6:28 PM to test gethtmlcodestring and its needed change to getopcode
*)
...
BEGIN
  ...
  LOOP
    ...
    INI1TKN(tpv1,INBUF);
    LOOP
      TOKENPTR.GetHtmlCodeString(tpv1,TOKEN,tc,RETCOD);
      IF RETCOD > 0 THEN
        WriteString("GETTKN's RETCOD is ");
        WriteCard(RETCOD);
        WriteLn;
      ...
      WriteString(', terminating code= ');
      IF tc THEN WriteString('yes ') ELSE WriteString('no') END;
      WriteLn;
      WriteLn;
    END; (* Loop for html code strings *)      
  END(*LOOP for lines *);
