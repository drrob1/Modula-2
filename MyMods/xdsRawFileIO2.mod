...
IMPORT  SeqFile, STextIO, SWholeIO, IOResult,(*TextIO, WholeIO,*) RawIO, SLongIO;
...
(*  str5    : ARRAY [1..5] OF CHAR; *)
  realarray : ARRAY [1..10] OF LONGREAL;
BEGIN
  FOR i:= 1 TO 10 DO realarray[i] := VAL(LONGREAL,i) END;
...
(*    FOR i:=0 TO 9 DO *)
      RawIO.Write(out.handle,realarray);
...
(*
  IF in.openRes = SeqFile.opened THEN
    ...
    STextIO.WriteLn;
    FOR i:=0 TO 9 DO
      RawIO.Read(in.handle,realarray);
...
*)
  IF in.openRes = SeqFile.opened THEN
    ...
    LOOP
      ...
      INC(c);
      FOR i := 1 TO 10 DO 
        SLongIO.WriteFixed(realarray[i],0,5);
        STextIO.WriteLn;
      END;
      STextIO.WriteString(' should only get here once');
      STextIO.WriteLn;
    END; 
    SeqFile.Close(in.handle);
