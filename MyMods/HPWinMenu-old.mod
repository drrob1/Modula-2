...
FROM HPCALC IMPORT STACKSIZE,PUSHX,READX,GETSTACK,DUMPSTACK,GETRESULT,RealStack,Holidays;
...
(*     WriteStringAt(tw,0,10,longstr,a); WriteLn(tw); *)
     ...
     WriteString(tw,longstr,a); WriteLn(tw);
     longstr := ' ROLLDN -- roll the stack down one register.  X goes to T1.';
     WriteString(tw,longstr,a); WriteLn(tw);
