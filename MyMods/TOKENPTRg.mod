IMPLEMENTATION MODULE TOKENPTRg;
(*
Copyright (C) 1987  Robert Solomon MD.  All rights reserved.
  ...
  20 Oct 11 -- Added underscore '_' as unary minus.  FSA state is dgt.
   2 Apr 13 -- I think I fixed a memory leak in that the init routines used NEW but I never called
  ...
  12 Oct 13 -- Noticed that this GETOPCODE does not recognize several operators that TKNRTNS does.  
                Other differences in GETOPCODE are noted as of 22 Jul 06.
*)
...
BEGIN
  IF tpv = CAST(TKNPTRTYP, NIL) THEN NEW(tpv); END;
  WITH tpv^ DO
    CURPOSN  := 3;
    TKNBUF   := BUF;
    PREVPOSN := 0;
...
BEGIN
  IF tpv = CAST(TKNPTRTYP, NIL) THEN
    NEW(tpv);
  END;
  WITH tpv^ DO
    CURPOSN  := 1;
...
VAR
    ...
    TEMP         : BUFTYP;
    OK,IsNeg     : BOOLEAN;
    convresult   : LongStr.ConvResults;
BEGIN
  RETCOD2 := 0;
  REALVAL := 0.0;
  WITH tpv^ DO
...
(*    TOKEN.CHARS[1] := '0'; Now handled at bottom of proc *)
    ...
    TOKEN.CHARS[TOKEN.COUNT+1] := NULL;
    TOKEN.LENGTH := TOKEN.COUNT;
    LongStr.StrToReal(TOKEN.CHARS,REALVAL,convresult);
    OK := convresult = LongStr.strAllRight;
    IF NOT OK THEN RETCOD2 := 7; END(*IF*);
    IF TOKEN.CHARS[TOKEN.COUNT] = '%' THEN
        REALVAL := REALVAL / 100.0;
    END;
  END(*with*);
END GETTKNREAL;
