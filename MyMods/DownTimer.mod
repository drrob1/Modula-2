...
20 Jan 05 -- Added icon and logic for some keys.
 1 Feb 05 -- Added WindowText.
  -----------------------------------------*)
%IF WIN32 %THEN
    <*/Resource:DownTimer.RES*>
%ELSE
%END
...
VAR
   ...
   bool            :  BOOLEAN;
   BeepParam,FlashTime,Color,CountDownTimer,MinutesLeft,SecondsLeft,HoursLeft,ColorVal,
     ColorValRed,ColorValBlue,ColorValGreen : CARDINAL;
   MinutesLeftStr,SecondsLeftStr,HoursLeftStr,WindowText : STRTYP;
...
*)
               ...
               Strings.Append(' -- ',WindowText);
               Strings.Append(szAppName,WindowText);
(*  The Title gets truncated by a lot of processes on taskbar.  Need to append szAppName.
               Strings.Insert(' -- ',0,WindowText);
               Strings.Insert(szAppName,0,WindowText);
*)
...
(*  wc.hIcon         := WINUSER.LoadIcon (NIL, WINUSER.IDI_APPLICATION^); *)
  wc.hIcon         := WINUSER.LoadIcon (WINX.Instance, '#200');
  wc.hCursor       := WINUSER.LoadCursor (NIL, WINUSER.IDC_ARROW^);
  wc.hbrBackground := SYSTEM.CAST(WIN32.HBRUSH, WINGDI.GetStockObject (WINGDI.WHITE_BRUSH));
  wc.lpszMenuName  := NIL;
  wc.lpszClassName := SYSTEM.ADR(szAppName);
(*  wc.hIconSm       := WINUSER.LoadIcon (NIL,WINUSER.IDI_APPLICATION^); *)
  wc.hIconSm       := WINUSER.LoadIcon (WINX.Instance, '#200');
