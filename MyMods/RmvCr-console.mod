...
(*
  To Do
  MyFIO2 for file read as this creates buffers that have to be passed.  I'll have to make them VAR INOUT.
  Right now the file name code does not work properly.  And I have to remember that filenames can have
  opcode chars like - in them.  I don't think I make _ an opcode char.
*)
...
VAR
  ...
  TKNSTATE                                     : FSATYP;
  str1,str,s,s1,s2,s3,s4                       : STRTYP;
  I,J                                          : INTEGER;
  infile,outfile                               : File; (* FileFunc exported type *)
  innameparts,outnameparts                     : FileNameParts;
  inputline, inname, outname                   : ARRAY [0..255] OF CHAR;
  InBuf, OutBuf, LongInputLine                 : ARRAY [1..8*1024] OF CHAR;
...
*)
  ...
  IF c = 0 THEN
    ...
    IF LENGTH(OutBuf) = 0 THEN
      ConstructFileName(InBuf,'.out',outname);
    ELSE
    	ConstructFileName(OutBuf,'.out',outname);
    END;

  ELSE
  	ConstructFileName(inputline,'*.txt',inname);
  	ConstructFileName(inputline,'*.out',outname);
