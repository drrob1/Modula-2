...
BEGIN
    ...
    *)
    ...
    ZapDigits(ADR(res^.digits[res^.used]), max-res^.used);
    res^.used := max;
...
BEGIN
    ...
    REPEAT
        ...
        FOR j := SIZE(Digit)-1 TO 0 BY -1 DO
            ...
            IF i+1 <= HIGH(text) THEN
                text[i] := str[0];
                text[i+1] := str[1];
                i := i + 2;
            ELSIF i <= HIGH(text) THEN
                text[i] := str[0];
                INC(i);
            END;
        END;
